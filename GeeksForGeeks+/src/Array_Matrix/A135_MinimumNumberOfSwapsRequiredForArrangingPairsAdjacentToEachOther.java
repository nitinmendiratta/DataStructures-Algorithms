 ...
 */
public class A135_MinimumNumberOfSwapsRequiredForArrangingPairsAdjacentToEachOther {

	// This function updates indexes of elements 'a' and 'b'
	public void updateindex(int index[], int a, int ai, int b, int bi){
	    index[a] = ai;
	    index[b] = bi;
	}
	...
	public int minSwapsUtil(int arr[], int pairs[], int index[], int i, int n){
	...
	public int minSwaps(int n, int pairs[], int arr[]){
	...
	public static void main(String[] args) {
		A135_MinimumNumberOfSwapsRequiredForArrangingPairsAdjacentToEachOther obj = new A135_MinimumNumberOfSwapsRequiredForArrangingPairsAdjacentToEachOther();
	    ...
	    int arr[] = {0, 3, 5, 6, 4, 1, 2};
	    int pairs[] = {0, 1,3, 2,6, 4,5};
	    int n = arr.length/2;  // Number of pairs n is half of total elements
	 
	    // If there are n elements in array, then
	    // there are n pairs
	    System.out.println("Min swaps required is "+obj.minSwaps(n, pairs, arr));
	}
