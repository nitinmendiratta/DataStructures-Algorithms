 ...
 * 
 */
public class A097_StableMarriageProblem {

	// This function returns true if woman 'w' prefers man 'm1' over man 'm'
	public boolean wPrefersM1OverM(int prefer[][], int w, int m, int m1){
	...
	public void stableMarriage(int prefer[][]){
		int N = prefer.length/2;
		// Stores partner of women. This is our output array that stores paring information.  The value of wPartner[i]
		// indicates the partner assigned to woman N+i.  Note that the woman numbers between N and 2*N-1. The value -1
		...
		while (freeCount > 0){
			...
			for (int i = 0; i < N && mFree[m] == false; i++){
				int w = prefer[m][i];
				// The woman of preference is free, w and m become partners (Note that the partnership maybe changed
				...
				else{  // If w is not free Find current engagement of w
					int m1 = wPartner[w-N];
	...
	public static void main(String[] args) {
		A097_StableMarriageProblem obj = new A097_StableMarriageProblem();
		int prefer[][] = { 
				{7, 5, 6, 4},
				{5, 4, 6, 7},
				{4, 5, 6, 7},
